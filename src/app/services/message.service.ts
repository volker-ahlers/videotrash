...
import {
    CopyObject,
    converNumbersFromTextToConcattedInteger,
} from '../more/helper';
...
export class MessageService {
    ...
    setMessagesGroupedByGate(gates: any, prepareByStep: boolean = false) {
        ...
        if (prepareByStep) this.prepareMessagesByStep();
        else this.prepareMessages();
        this.debug && console.log(this.messagesGroupedByGate());
    }
    ...
    async getStreamsFromDocker(): Promise<any> {
        ...
        try {
            response = await firstValueFrom(
                this.httpClient.get<any>(environment.docker_rest_service_url)
            );
        } catch {
            // TODO Dummidata
            ...
            catchError(
                this.error.handleError('GetStreamsFromDocker', undefined)
            );
        } finally {
    ...
    sortStreams(streams: IStream[]) {
        ...
        streams.sort((a: any, b: any) =>
            a.sortNr < b.sortNr ? -1 : a.sortNr > b.sortNr ? 1 : 0
        );
    ...
    async getMessagesFromStorage(): Promise<void> {
        try {
            ...
            const response: Promise<IMessage[]> = await firstValueFrom(
                this.httpClient.get<any>(environment.docker_message_storage_url)
            );
            ...
            catchError(
                this.error.handleError('restoreMessagesFromStorage', undefined)
            );
        }
    }
    ...
    getMessagesFromLocalStorage(): void {
        let sessString = localStorage.getItem('gates') ?? '{}',
            gates,
            e;
        try {
    ...
    saveMessagesToStorage(): void {
        this.debug &&
            console.log('saveMessagesToStorage', this.messagesGroupedByGate());
        localStorage.setItem(
            'gates',
            JSON.stringify(this.messagesGroupedByGate())
        );
    }
    ...
    prepareMessagesByStep() {
        const msgs = this.messagesGroupedByGate()[this.sensorLabel()]?.filter(
            (msg: { state: string }) => msg.state === AlarmStates.Active
        );
        const step = this.objectStepSig();
        this.debug &&
            console.log(
                `prepareMessagesByStep with step ${step} label ${this.sensorLabel()} in messages`,
                msgs
            );
        ...
        try {
            ...
            console.log(
                `error on prepareMessagesByStep with step ${step} label ${this.sensorLabel()} in messages ${msgs}`
            );
        }
    }
    ...
    */
        ...
        this.debug &&
            console.log(msgs, this.sensorLabel(), this.messagesGroupedByGate());
        const msg_id = this.selectedMessage()
            ? this.selectedMessage()?.message_id
            : null;
        this.debug &&
            console.log(
                `prepareMessages with messageID ${msg_id} label ${this.sensorLabel()} in messages`,
                msgs
            );
        try {
            if (msgs) this.setMessagesPerGateSig(msgs);
            if (msg_id)
                this.setObjectStepSignal(
                    msgs
            ...
            console.log(
                `error on prepareMessages with messageID ${msg_id} label ${this.sensorLabel()} in messages ${msgs}`
            );
        }
    }
    ...
    getActiveMessagesCnt(gate: string, component?: string): number {
        this.debug &&
            console.log(
                'getActiveMessagesCnt',
                component,
                this.messagesGroupedByGate()
            );
        try {
            return this.messagesGroupedByGate()[gate]?.filter(
                (msg: { state: string }) => msg.state === AlarmStates.Active
            ).length;
        } catch (e: unknown) {
            console.error(e);
    ...
    addMessageToGates(message: IMessage) {
        this.debug &&
            console.log('addMessageToGates', this.messagesGroupedByGate());
        ...
        gatesObj[message.sensor_label.toString()].sort((a: any, b: any) =>
        ...
        this.snackBarService.snackBarInfo(
            'Incoming Message at ' + message.sensor_label
        );
    ...
    markMessageFromGate(gate: string, message_id: string, category: string) {
        ...
        if (gates[gate]) {
            ...
            this.snackBarService.snackBarSuccess(
                'Message marked as cleared from Gate ' + gate
            );
    ...
    checkIfStreamExists(gate: string): boolean {
        const stream = this.streams()?.filter(
            (stream: { name: string }) => stream.name === gate
        );
        this.debug && console.log(stream, stream.length);
        if (stream.length === 0) {
            if (gate !== '')
                this.snackBarService.snackBarFail(
                    `Message for Gate ${gate} not added, gate not found`
                );
            return false;
        }
        return true;
    ...
    convertToAssociativeObject(obj: any) {
        ...
        this.debug &&
            console.log('convertJsonToAssociativeObject', messagesTMP);
    ...
    convertAssociativeToArrayForCockpit(gatesObj: any) {
        ...
        messagesArray.sort((a: any, b: any) =>
            a.sortNr < b.sortNr ? -1 : a.sortNr > b.sortNr ? 1 : 0
        );
     ...
     */
    async messageHandleService(
        gate: string,
        message_id: string,
        category: string
    ): Promise<void> {
        try {
            const http: any = await firstValueFrom(
                this.httpClient.post<any>(
                    `${environment.docker_message_update_url}/${category}/${message_id}`,
                    {}
                )
            );
            ...
            catchError(
                this.error.handleError('upsertMessagesFromStorage', undefined)
            );
            if (!environment.production) {
                //testcases
                this.markMessageFromGate(gate, message_id, category);
    ...
    async setTimeslot(timeslot: ITimeslot) {
        try {
            const http: any = await firstValueFrom(
                this.httpClient.post<any>(
                    `${environment.docker_message_timeslot_url}`,
                    timeslot
                )
            );
    ...
    async getObjectLabels() {
        try {
            this.objectLabels = await firstValueFrom(
                this.httpClient.get<string[]>(
                    `${environment.docker_message_labels_url}`
                )
            );
    ...
    async getDummiStreams(): Promise<any> {
        try {
            return await firstValueFrom(
                this.httpClient.get<any>(`assets/streamslikeDocker.json`)
            );
        } catch {
